# 1.0.0

| **Release Date**                 | April 18, 2016 |
|----------------------------------|-----------------|
| **Plug-In Version**              | 1.0.0           |
| **Supported nopCommerce Version** | 3.50, 3.60, 3.70 |

**Features:**

1. Admin mobile number configuration  
2.  Individual messages to customers from the customer edit and order edit pages.  
3. Multiple provider settings and country-wise provider settings.  
4. Email SMS API, HTTP SMS API, and Twilio Voice & SMS API configurations.  
5. Template-based messages. 

6. Customer events  
   6.1 Mobile number verification  
   6.2 Order placed  
   6.3 Shipment initiated  
   6.4 Order delivered  
   6.5 Order cancelled  
   6.6 Order note added  
   6.7 Order processing  
   6.8 Payment refund  

7. Admin events  
   7.1 New customer registration  
   7.2 New order placed in store  

[← Previous](1.1.0.md) | [Next →](installation.md)